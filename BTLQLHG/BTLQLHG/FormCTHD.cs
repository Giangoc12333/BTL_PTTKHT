...
namespace BTLQLHG
{
    public partial class FormCTHD : Form
    {
        ...
        {
            command = connection.CreateCommand();
            command.CommandText = "Select * from CTHD Where TenKH like N'%" + TimKiem.Text.Trim() + "%'";
            adapter.SelectCommand = command;
            table.Clear();
            adapter.Fill(table);
            dgvThongTinHD.DataSource = table;
        ...
        {
            string sql = "select sum(TongTien) as TongTien From CTHD ";
            SqlCommand cmd = new SqlCommand(sql, connection);
            SqlDataReader dr;
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                txtTHD.Text = dr["TongTien"].ToString();
            }
            dr.Close();
        }
